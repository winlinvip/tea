...
#include <unistd.h>
#include "libbpf_stun_netem_kern.skel.h"

struct BpfHandler {
    libbpf_stun_netem_kern* obj;
    bpf_tc_hook* tc_hook;
    bpf_tc_opts* tc_opts;
    BpfHandler(libbpf_stun_netem_kern* obj) {
        this->obj = obj;
        tc_hook = NULL;
        tc_opts = NULL;
    ...
    ~BpfHandler() {
        ...
        if (obj) libbpf_stun_netem_kern__destroy(obj);
...
int main(int argc, char** argv) {
    ...
    BpfHandler hdr(libbpf_stun_netem_kern__open_opts(&opts));
    if (!hdr.obj) {
    ...
    int r0;
    if ((r0 = libbpf_stun_netem_kern__load(hdr.obj)) != 0) {
        printf("load ebpf failed, r0=%d\n", r0);
        return 1;
    }
    if ((r0 = libbpf_stun_netem_kern__attach(hdr.obj)) != 0) {
    ...
    printf("Apply netem to all STUN packets...\n");
